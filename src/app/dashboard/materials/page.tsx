...
/**
 * Modern Materials Stream Picker Page
 *
 * First step in material management workflow
 * Features:
...
import { ArrowLeft, BookOpen, GraduationCap, TrendingUp, AlertCircle, ChevronRight, FolderOpen } from 'lucide-react'
...
// Stream-specific styling for materials
...
export default function MaterialsStreamPickerPage() {
  ...
  const fetchStreams = async () => {
    try {
      ...
      console.error('[MATERIALS_STREAM_PICKER_ERROR]', err)
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <h1 className="text-4xl font-bold text-neutral-900">Manage Study Materials</h1>
        <p className="text-lg text-neutral-600">
          Select a stream to organize and upload materials
        </p>
      </motion.div>
      {/* Error State */}
      {streamsError && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {streams.map((stream, index) => {
            ...
            return (
              ...
              >
                <Link href={`/dashboard/materials/stream/${stream.id}`}>
                  <div className="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-neutral-200 hover:border-transparent">
                    ...
                    <div className="relative p-8">
                      ...
                      <p className="text-sm text-neutral-600 group-hover:text-white/90 transition-colors duration-300 mb-6">
                        Manage and organize study materials for {stream.name}
                      </p>
